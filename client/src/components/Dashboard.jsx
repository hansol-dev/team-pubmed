    ...
    <section id="chat" className={`tab-panel ${active ? "is-active" : ""}`}>
      <div className="chat-layout">
        ...
        <article className="chat-card clay-card">
          ...
          <div className="chat-log">{!conversationId || !messages.length ? <div className="chat-message assistant intro-message"><span className="avatar">✦</span><div>{conversationId ? (currentPapers.length ? INTRO : "새 채팅입니다. 궁금한 연구 주제나 논문 분석 방향에 대해 질문해보세요.") : "논문 채팅방을 선택하거나 새 채팅을 시작해주세요."}</div></div> : messages.map((message, index) => <div className={`chat-message ${message.role}`} key={message.id ?? index}><span className="avatar">{message.role === "assistant" ? "✦" : "나"}</span><div className="message-content"><ReactMarkdown remarkPlugins={[remarkGfm]}>{message.content}</ReactMarkdown></div></div>)}</div>
          <form className="chat-form" onSubmit={send}><input value={input} onChange={(event) => setInput(event.target.value)} placeholder={conversationId ? (currentPapers.length ? "선택한 논문에 대해 질문하세요" : "연구 주제나 논문 분석 방향에 대해 질문하세요") : "먼저 채팅방을 열어주세요"} disabled={!conversationId || sending} required /><button className="primary-button" disabled={!conversationId || sending}>보내기 <span>→</span></button></form>
