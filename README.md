...
- Research Galaxy는 내 관심 논문을 프로젝트별 그래프로 만들고, 검색된 서브그래프만 근거로 AI 답변을 생성합니다.
...
| Research Galaxy | 내 관심 논문을 전체·프로젝트·미분류 범위로 탐색하고, 관련 서브그래프를 근거로 AI 답변을 생성합니다. |
...
`지식 그래프` 메뉴는 인증된 사용자의 활성 관심 논문을 독립된 전체 화면에
표시합니다. 헤더에서 `전체 관심 논문 / 미분류 / 프로젝트` 범위를 바꾸면
그래프와 AI 답변 근거가 함께 바뀝니다. 기존 PubMed 148편 코퍼스는 운영
데이터와 섞지 않고 `데모 데이터`를 선택했을 때만 사용합니다.
...
| 구성 요소 | 의미 |
| --- | --- |
| 연구 주제 노드 | 관심 논문의 제목·초록에서 분류한 의학 연구 분야의 중심점 |
| 논문 노드 | 선택 범위의 활성 관심 논문. 화면에는 `P001` 같은 식별자로 간결하게 표시 |
...
그래프는 별도 그래프 DB에 복제하지 않습니다. `user_paper_collections`,
`project_papers`, `research_projects`에서 현재 사용자의 `is_del = false` 행만
읽고, 공용 `pubmed_records`의 제목·초록으로 매 요청 시 구성합니다.

#### 질문이 Graph RAG 답변이 되는 과정

1. 선택한 범위에서 최근 관심 논문 최대 200편을 불러옵니다.
2. 질문과 제목·초록·저널·연구 주제·핵심 개념을 비교해 직접 근거를 최대 3편 찾습니다.
3. 직접 근거와 주제·개념·내용 유사성을 공유하는 그래프 이웃을 더해 최대 6편의 서브그래프를 만듭니다.
4. 이 서브그래프의 제목·초록과 연결 이유만 LangGraph 안전 파이프라인을 거쳐 LLM에 전달합니다.
5. 답변과 함께 `직접 근거 / 그래프 이웃`, PMID, 논문 제목을 표시합니다.
6. 근거 논문을 누르면 해당 그래프 노드와 상세 정보로 이동합니다.

질문을 입력하기 전의 그래프 구성에는 OpenAI API를 사용하지 않습니다.
`Graph RAG 답변`을 요청할 때만 LLM API가 호출됩니다. 현재 Graph RAG의
증거 범위는 **제목+초록**이며 PMC 전문과 업로드 PDF는 아직 사용하지
않습니다. 전문 기반 질의는 별도의 `AI 챗봇` 벡터 RAG가 담당합니다.
...
- 전체 관심 논문·미분류·프로젝트별 그래프 범위 전환
- 자연어 질문의 관련 논문을 찾고 주제·핵심 개념·유사 연구 이웃까지 확장한 AI 답변 생성
...
네. 현재 Research Galaxy는 검색된 서브그래프를 LLM 컨텍스트로 전달하고,
그 근거 안에서 답변을 생성하는 Graph RAG 흐름까지 연결되어 있습니다.

```text
Research Galaxy
관심 논문 범위 → 직접 근거 검색 → 그래프 이웃 확장
→ 제목·초록 서브그래프를 LLM에 전달 → 답변·출처 표시

AI 챗봇 탭
질문 → 임베딩 생성 → pgvector 청크 검색 → 관련 문단을 LLM에 전달 → 답변 생성
```

두 RAG는 근거 단위가 다릅니다. Research Galaxy는 여러 관심 논문의
`제목+초록+관계`를 비교하는 탐색에 적합하고, AI 챗봇은 선택한 1~5편의
`원문 청크`를 자세히 읽는 질문에 적합합니다.
...
| 관심 논문 그래프를 요청 시 구성 | 논문 메타데이터를 중복 저장하지 않고 사용자·프로젝트·소프트 삭제 상태를 즉시 반영 |
| 데모 그래프 분리 | 운영 사용자 데이터와 148편 시연 코퍼스를 명확히 구분 |
| Vercel same-origin API | 프론트엔드와 API를 한 도메인에서 제공해 CORS와 배포 구성을 단순화 |

Research Galaxy는 관심 논문의 제목·초록 서브그래프를 LLM에 전달하고,
실제 논문 채팅은 선택 논문의 원문·초록 청크를 `pgvector`로 검색합니다.
...
├── shared/
│   ├── wordCloud.js          # 브라우저·서버가 공유하는 제목+초록 키워드 계산
│   └── researchGraph.js      # 관심 논문 주제·개념 분류와 서브그래프 검색
├── supabase/
...
- Research Galaxy는 한 번에 최근 관심 논문 최대 200편을 표시함
- Research Galaxy 답변 근거는 제목+초록이며 PMC 전문·업로드 PDF는 아직 사용하지 않음
